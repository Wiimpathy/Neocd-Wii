...
/*** Used for SMB testing ***/
#ifdef USESMB
#include "ctests.h"
static int ct_count = 92;
#endif
...
#define PRG_MEM ( 2 * 1024 * 1024 )
#define SPR_MEM ( 4 * 1024 * 1024 )
#define PCM_MEM ( 1024 * 1024 )
#define ROM_MEM ( 512 * 1024 )
#define FIX_MEM ( 128 * 1024 )
#define IPL_MEM 28672
/*** 0.1.46 - Code reduction ***/
#define MEM_BUCKET (PRG_MEM + SPR_MEM + PCM_MEM + ROM_MEM + FIX_MEM + IPL_MEM)
...
void memdebug()
{
    ...
    fp = GEN_fopen("program.bin", "wb");
    GEN_fwrite(neogeo_prg_memory, 1, PRG_MEM, fp);
    GEN_fclose(fp);

    fp = GEN_fopen("rom.bin", "wb");
    GEN_fwrite(neogeo_rom_memory, 1, ROM_MEM, fp);
    GEN_fclose(fp);

    fp = GEN_fopen("sprite.bin", "wb");
    GEN_fwrite(neogeo_spr_memory, 1, SPR_MEM, fp);
    GEN_fclose(fp);

    fp = GEN_fopen("pcm.bin", "wb");
    GEN_fwrite(neogeo_pcm_memory, 1, PCM_MEM, fp);
    GEN_fclose(fp);

    fp = GEN_fopen("fix.bin", "wb");
    GEN_fwrite(neogeo_fix_memory, 1, FIX_MEM, fp);
    GEN_fclose(fp);

    fp = GEN_fopen("z80.bin", "wb");
    GEN_fwrite(subcpu_memspace, 1, 0x10000, fp);
    GEN_fclose(fp);

    fp = GEN_fopen("fix_usage.bin", "wb");
    GEN_fwrite(video_fix_usage, 1, 4096, fp);
    GEN_fclose(fp);
...
int neogeo_redux(void)
{
    ...
    if (!fp) {
      ActionScreen((char *) "NO BIOS AVAILABLE!");
      neogeocd_exit();
    }
...
#ifdef USESMB
    /*** SMB Testing Only ***/
    strcpy(basedir,"/NeoGeoCD/");
    strcat(basedir,smbsubs[ct_count++]);
#endif
...
{
    InfoScreen("NEOGEO: Exit requested by software...");
    while (1);
}
...
static void neogeo_run(void)
{
  /*** Let's Go !!! ***/
    StartGX();
    InitGCAudio();
mixer_set( config.SoundVolume, config.Mp3Volume, config.Mp3Low, config.Mp3Mid, config.Mp3High);
reset_video(config.Tvmode);
    FrameTicker = 0;
	

  /*** Do emulation - FOREVER! ***/
    for (;;) {
  neogeo_run_bios();
  restart = 0;
...
static void neogeo_do_cdda(int command, int track_number_bcd)
{
  ...
  subcpu_memspace[offset + 1] = 0;
    ...
    case 7:
  track_number =
      ((track_number_bcd >> 4) * 10) + (track_number_bcd & 0x0F);

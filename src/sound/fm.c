...
/* DT2=2 *SQL(2.5) */
    ...
    ML(21.98), ML(23.55),
/* DT2=3 *SQL(3)   */
    ML(0.87), ML(1.73), ML(3.46), ML(5.19), ML(6.92), ML(8.65),
    ML(10.38),
    ML(12.11),
...
static void print_struct(FILE * f, YM2610 * F2610, Uint8 * data)
{
	...
	if (data == &F2610->CH[i])
	    fprintf(f, " - CH[%d] - ", i);
	for (j = 0; j < 4; j++) {
	    if (data == &F2610->CH[i].SLOT[j])
		fprintf(f, " - SLOT[%d] - ", j);
	    if (data == &F2610->CH[i].SLOT[j].DT)
		fprintf(f, " - SL.DT - ");
	    if (data == &F2610->CH[i].SLOT[j].DT2)
		fprintf(f, " - SL.DT2 - ");
	    if (data == &F2610->CH[i].SLOT[j].TL)
		fprintf(f, " - SL.TL - ");
	    if (data == &F2610->CH[i].SLOT[j].KSR)
		fprintf(f, " - SL.KSR - ");
	    if (data == &F2610->CH[i].SLOT[j].AR)
		fprintf(f, " - SL.AR - ");
	    if (data == &F2610->CH[i].SLOT[j].DR)
		fprintf(f, " - SL.DR - ");
	    if (data == &F2610->CH[i].SLOT[j].SR)
		fprintf(f, " - SL.SR - ");
	    if (data == &F2610->CH[i].SLOT[j].RR)
		fprintf(f, " - SL.RR - ");
	    if (data == &F2610->CH[i].SLOT[j].SL)
		fprintf(f, " - SL.SL - ");
	    if (data == &F2610->CH[i].SLOT[j].SEG)
		fprintf(f, " - SL.SEG - ");
	    if (data == &F2610->CH[i].SLOT[j].ksr)
		fprintf(f, " - SL.KSR2 - ");
	    if (data == &F2610->CH[i].SLOT[j].mul)
		fprintf(f, " - SL.MUL - ");
	    if (data == &F2610->CH[i].SLOT[j].Cnt)
		fprintf(f, " - SL.CNT - ");
	    if (data == &F2610->CH[i].SLOT[j].Incr)
		fprintf(f, " - SL.INCR - ");
	    if (data == &F2610->CH[i].SLOT[j].state)
		fprintf(f, " - SL.STATE - ");
	    if (data == &F2610->CH[i].SLOT[j].eg_next)
		fprintf(f, " - SL.EG_NEXT - ");
	    if (data == &F2610->CH[i].SLOT[j].evc)
		fprintf(f, " - SL.evc - ");
	    if (data == &F2610->CH[i].SLOT[j].eve)
		fprintf(f, " - SL.eve - ");
	    if (data == &F2610->CH[i].SLOT[j].evs)
		fprintf(f, " - SL.evs - ");
	    if (data == &F2610->CH[i].SLOT[j].evsa)
		fprintf(f, " - SL.evsa - ");
	    if (data == &F2610->CH[i].SLOT[j].evsd)
		fprintf(f, " - SL.evsd - ");
	    if (data == &F2610->CH[i].SLOT[j].evss)
		fprintf(f, " - SL.evss - ");
	    if (data == &F2610->CH[i].SLOT[j].evsr)
		fprintf(f, " - SL.evsr - ");
	    if (data == &F2610->CH[i].SLOT[j].TLL)
		fprintf(f, " - SL.TLL - ");
	    if (data == &F2610->CH[i].SLOT[j].amon)
		fprintf(f, " - SL.AMON - ");
	    if (data == &F2610->CH[i].SLOT[j].ams)
		fprintf(f, " - SL.AMS - ");



	}
	if (data == &F2610->CH[i].PAN)
	    fprintf(f, " - PAN - ");
	if (data == &F2610->CH[i].ALGO)
	    fprintf(f, " - ALGO - ");
	if (data == &F2610->CH[i].FB)
	    fprintf(f, " - FB - ");
	if (data == &F2610->CH[i].op1_out)
	    fprintf(f, " - OP1OUT - ");
	if (data == &F2610->CH[i].connect1)
	    fprintf(f, " - CONNECT1 - ");
	if (data == &F2610->CH[i].connect2)
	    fprintf(f, " - CONNECT2 - ");
	if (data == &F2610->CH[i].connect3)
	    fprintf(f, " - CONNECT3 - ");
	if (data == &F2610->CH[i].connect4)
	    fprintf(f, " - CONNECT4 - ");
	if (data == &F2610->CH[i].pms)
	    fprintf(f, " - PMS - ");
	if (data == &F2610->CH[i].ams)
	    fprintf(f, " - AMS - ");
	if (data == &F2610->CH[i].fc)
	    fprintf(f, " - FC - ");
	if (data == &F2610->CH[i].fn_h)
	    fprintf(f, " - FN_H - ");
	if (data == &F2610->CH[i].kcode)
	    fprintf(f, " - KCODE - ");
    }
    if (data == &F2610->address1)
	fprintf(f, " - ADD1 - ");
    if (data == &F2610->pcmbuf)
	fprintf(f, " - PCMBUF - ");
    if (data == &F2610->adpcmTL)
	fprintf(f, " - ADPCMTL - ");
    for (i = 0; i < 6; i++)
	if (data == &F2610->adpcm[i])
	    fprintf(f, " - ADPCM[%d] - ", i);
    if (data == &F2610->adpcmreg)
	fprintf(f, " - ADPCMREG - ");
    if (data == &F2610->adpcm_arrivedEndAddress)
	fprintf(f, " - ADPCM_END - ");
    if (data == &F2610->deltaT)
	fprintf(f, " - DELTAT - ");
...
static void
init_timetables(FM_ST * ST, Uint8 * DTTABLE, int ARRATE, int DRRATE)
{
    int i, d;
    double rate;
...
static void FMsave_state_channel(FM_CH * CH, int num_ch)
{
    ...
    for (ch = 0; ch < num_ch; ch++, CH++) {
	/* channel */
	create_state_register(ST_YM2610_FM, "op1_out1", ch + 1,
			      &CH->op1_out[0], 1 * sizeof(Sint32), REG_INT32);
	create_state_register(ST_YM2610_FM, "op1_out2", ch + 1,
			      &CH->op1_out[1], 1 * sizeof(Sint32), REG_INT32);
	create_state_register(ST_YM2610_FM, "phasestep", ch + 1, &CH->fc,
			      1 * sizeof(Uint32), REG_UINT32);
	create_state_register(ST_YM2610_FM, "fn_h", ch + 1, &CH->fn_h,
			      1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_FM, "kcode", ch + 1, &CH->kcode,
			      1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_FM, "PAN", ch + 1, &CH->PAN,
			      1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_FM, "ALGO", ch + 1, &CH->ALGO,
			      1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_FM, "FB", ch + 1, &CH->FB,
			      1 * sizeof(Uint8), REG_UINT8);

	/* slots */
	for (slot = 0; slot < 4; slot++) {
	    FM_SLOT *SLOT = &CH->SLOT[slot];
	    create_state_register(ST_YM2610_FM, "phasecount",
				  (ch << 4) | slot, &SLOT->Cnt,
				  1 * sizeof(Uint32), REG_UINT32);
	    create_state_register(ST_YM2610_FM, "Incr", (ch << 4) | slot,
				  &SLOT->Incr, 1 * sizeof(Uint32),
				  REG_UINT32);
	    create_state_register(ST_YM2610_FM, "ksr", (ch << 4) | slot,
				  &SLOT->ksr, 1 * sizeof(Uint8), REG_UINT8);
	    create_state_register(ST_YM2610_FM, "evc", (ch << 4) | slot,
				  &SLOT->evc, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "eve", (ch << 4) | slot,
				  &SLOT->eve, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "evs", (ch << 4) | slot,
				  &SLOT->evs, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsa", (ch << 4) | slot,
				  &SLOT->evsa, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsd", (ch << 4) | slot,
				  &SLOT->evsd, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "evss", (ch << 4) | slot,
				  &SLOT->evss, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsr", (ch << 4) | slot,
				  &SLOT->evsr, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "TLL", (ch << 4) | slot,
				  &SLOT->TLL, 1 * sizeof(Sint32), REG_INT32);
	    create_state_register(ST_YM2610_FM, "TL", (ch << 4) | slot,
				  &SLOT->TL, 1 * sizeof(Uint32), REG_INT32);
...
{
    create_state_register(ST_YM2610_FM, "address", 1, &ST->address,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610_FM, "IRQ", 1, &ST->irq, 1 * sizeof(Uint8),
			  REG_UINT8);
    create_state_register(ST_YM2610_FM, "IRQ MASK", 1, &ST->irqmask,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610_FM, "status", 1, &ST->status,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610_FM, "mode", 1, &ST->mode,
			  1 * sizeof(Uint32), REG_UINT32);
    create_state_register(ST_YM2610_FM, "prescaler", 1, &ST->prescaler_sel,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610_FM, "freq latch", 1, &ST->fn_h,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610_FM, "TIMER A", 1, &ST->TA,
			  1 * sizeof(Sint32), REG_INT32);
    create_state_register(ST_YM2610_FM, "TIMER Acnt", 1, &ST->TAC,
			  1 * sizeof(Sint32), REG_INT32);
    create_state_register(ST_YM2610_FM, "TIMER B", 1, &ST->TB,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610_FM, "TIMER Bcnt", 1, &ST->TBC,
			  1 * sizeof(Sint32), REG_INT32);
...
static void OPNSetPris(FM_OPN * OPN, int pris, int TimerPris, int SSGpris)
{
    ...
    /* frequency base */
    OPN->ST.freqbase =
	(OPN->ST.rate) ? ((double) OPN->ST.clock / OPN->ST.rate) / pris : 0;
    /* Timer base time */
	...
	OPN->FN_TABLE[i] =
	    (Uint32) ((double) i * OPN->ST.freqbase * FREQ_RATE * (1 << 7) /
		      2);
    }
#if FM_LFO_SUPPORT
    /* LFO freq. table */
...
static void FMsave_state_adpcma(ADPCM_CH * adpcm)
{
	...
	create_state_register(ST_YM2610_ADPCMA, "flag", ch + 1, &adpcm->flag,
			      1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_ADPCMA, "flag_mask", ch + 1,
			      &adpcm->flagMask, 1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_ADPCMA, "now_data", ch + 1,
			      &adpcm->now_data, 1 * sizeof(Uint8), REG_UINT8);
	create_state_register(ST_YM2610_ADPCMA, "now_addr", ch + 1,
			      &adpcm->now_addr, 1 * sizeof(Uint32),
			      REG_UINT32);
	create_state_register(ST_YM2610_ADPCMA, "now_step", ch + 1,
			      &adpcm->now_step, 1 * sizeof(Uint32),
			      REG_UINT32);
	create_state_register(ST_YM2610_ADPCMA, "IL", ch + 1, &adpcm->IL,
			      1 * sizeof(Sint32), REG_INT32);

	create_state_register(ST_YM2610_ADPCMA, "step", ch + 1, &adpcm->step,
			      1 * sizeof(Uint32), REG_UINT32);
	create_state_register(ST_YM2610_ADPCMA, "start", ch + 1,
			      &adpcm->start, 1 * sizeof(Uint32), REG_UINT32);
	create_state_register(ST_YM2610_ADPCMA, "end", ch + 1, &adpcm->end,
			      1 * sizeof(Uint32), REG_UINT32);

	create_state_register(ST_YM2610_ADPCMA, "adpcmx", ch + 1,
			      &adpcm->adpcmx, 1 * sizeof(Sint32), REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "adpcmd", ch + 1,
			      &adpcm->adpcmd, 1 * sizeof(Sint32), REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "adpcml", ch + 1,
			      &adpcm->adpcml, 1 * sizeof(Sint32), REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "volume", ch + 1,
			      &adpcm->volume, 1 * sizeof(Sint32), REG_INT32);
    }
...
//    create_state_register(ST_YM2610, "ym2610"   ,1, F2610   , sizeof(YM2610));
    create_state_register(ST_YM2610, "out_ch", 1, out_ch, 4 * sizeof(Sint32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in1", 1, &pg_in1, 1 * sizeof(Sint32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in2", 1, &pg_in2, 1 * sizeof(Sint32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in3", 1, &pg_in3, 1 * sizeof(Sint32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in4", 1, &pg_in4, 1 * sizeof(Sint32),
			  REG_INT32);


#if 1
    create_state_register(ST_YM2610, "regs", 1, F2610->REGS,
			  512 * sizeof(Uint8), REG_UINT8);
    FMsave_state_st(&FM2610.OPN.ST);
    FMsave_state_channel(FM2610.CH, 6);
    /* 3slots */
    create_state_register(ST_YM2610, "slot3fc", 1, F2610->OPN.SL3.fc,
			  3 * sizeof(Uint32), REG_UINT32);
    create_state_register(ST_YM2610, "slot3fh", 1, &F2610->OPN.SL3.fn_h,
			  1 * sizeof(Uint8), REG_UINT8);
    create_state_register(ST_YM2610, "slot3kc", 1, F2610->OPN.SL3.kcode,
			  3 * sizeof(Uint8), REG_UINT8);
    /* address register1 */
    create_state_register(ST_YM2610, "address1", 1, &F2610->address1,
			  1 * sizeof(Sint32), REG_INT32);
    create_state_register(ST_YM2610, "arrivedFlag", 1,
			  &F2610->adpcm_arrivedEndAddress, 1 * sizeof(Uint8),
			  REG_UINT8);
    /* rythm(ADPCMA) */
...
#if BUILD_OPNB
int
YM2610Init(int gcclock, int rate,
